...
class model():
    ...
    def net_b(self):
        # === PEC scattering: exact boundary + exact initial condition ===
        ...
        Ez_x1_t, Hx_x1_t, Hy_x1_t = self.pec_scattering_exact(x1, y_rep, t_rep)
        ...
        Ez_y0_t, Hx_y0_t, Hy_y0_t = self.pec_scattering_exact(x_rep, y0, t_rep2)
        ...
        Ez_y1_t, Hx_y1_t, Hy_y1_t = self.pec_scattering_exact(x_rep, y1, t_rep2)
        ...
        Ez0_t, Hx0_t, Hy0_t = self.pec_scattering_exact(xx, yy, tt0)
    ...
    def pec_scattering_exact(self, x, y, t, nx_mode=1, ny_mode=1):
        """
        PEC plane-wall scattering exact solution (total field).
        PEC wall is at x = x_min.

        x, y, t: torch tensor with same shape
        return: Ez, Hx, Hy
        """
        ...
        # incident / reflected phases
        theta_i = -kx * xr + ky * yr - omega * t
        theta_r =  kx * xr + ky * yr - omega * t

        Ez = torch.sin(theta_i) - torch.sin(theta_r)
        Hx = (ky / omega) * (torch.sin(theta_i) - torch.sin(theta_r))
        Hy = (kx / omega) * (torch.sin(theta_i) + torch.sin(theta_r))
    ...
    def maxwell_exact_and_error(self, N=None, t0=0.5):
        ...
        with torch.no_grad():
            ...
            Ez_e, Hx_e, Hy_e = self.pec_scattering_exact(xx, yy, tt)
