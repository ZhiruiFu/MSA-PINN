...
class model():
    ...
    def net_b(self):
        # === Traveling-wave: periodic boundary + exact initial condition ===
        t_b = torch.linspace(
        ...
        # 周期边界值匹配
        loss_bc = ((out_x0 - out_x1) ** 2).mean() + ((out_y0 - out_y1) ** 2).mean()

        # 初值 t=0: exact traveling-wave profile
        ...
        Ez0_target, Hx0_target, Hy0_target = self.traveling_wave_exact(xx, yy, tt0)

        loss_ic = ((Ez0 - Ez0_target) ** 2).mean() \
                + ((Hx0 - Hx0_target) ** 2).mean() \
                + ((Hy0 - Hy0_target) ** 2).mean()
    ...
    def traveling_wave_exact(self, x, y, t):
        """
        2D TE Maxwell periodic traveling wave exact solution.
        ...
        if Lx <= 0 or Ly <= 0:
            raise ValueError("Domain length must be positive for traveling-wave setting.")
        ...
        theta = kx * (x - self.x_min) + ky * (y - self.y_min) - omega * t

        Ez = torch.sin(theta)
        Hx = (ky / omega) * torch.sin(theta)
        Hy = -(kx / omega) * torch.sin(theta)
    ...
    def maxwell_exact_and_error(self, N=None, t0=0.5):
        ...
        with torch.no_grad():
            ...
            Ez_e, Hx_e, Hy_e = self.traveling_wave_exact(xx, yy, tt)
